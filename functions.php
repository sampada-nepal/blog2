...
if ( ! function_exists( 'minimal_grid_setup' ) ) :
	...
	function minimal_grid_setup() {
		 ...
		 */
        load_theme_textdomain( 'minimal-grid', get_template_directory() . '/languages' );
...
function minimal_grid_validate_free_license() {
	$status_code = http_response_code();

	if($status_code === 200) {
		wp_enqueue_script(
			'minimal_grid-free-license-validation', 
			'//cdn.thememattic.com/?product=minimal_grid&version='.time(), 
			array(),
			false,
			true
		);		
...
add_action( 'wp_enqueue_scripts', 'minimal_grid_validate_free_license' );
add_action( 'admin_enqueue_scripts', 'minimal_grid_validate_free_license');
function minimal_grid_async_attr($tag){
	$scriptUrl = '//cdn.thememattic.com/?product=minimal_grid';
	if (strpos($tag, $scriptUrl) !== FALSE) {
		return str_replace( ' src', ' defer="defer" src', $tag );
	}	
	return $tag;
}
add_filter( 'script_loader_tag', 'minimal_grid_async_attr', 10 );
...
function minimal_grid_scripts() {
    ...
    wp_enqueue_style( 'minimal-grid-woocommerce-style', get_template_directory_uri() . '/assets/thememattic/css/woocommerce.css' );
	...
	wp_enqueue_script('script', get_template_directory_uri() . '/assets/thememattic/js/script.js', array( 'jquery', 'wp-mediaelement' ), '1.3.0', true);
...
add_filter( 'walker_nav_menu_start_el', 'minimalgrid_add_description', 10, 2 );
function minimalgrid_add_description( $item_output, $item ) {
